...
stats_ns_all <- ddply(norm_ns_all, ~ VM + Benchmark, summarise,
               RR.sd = sd(RuntimeRatio),
               RuntimeFactor = mean(RuntimeRatio))  ## central estimator, is this 
stats_ns_all <- ddply(stats_ns_all, ~ VM, transform,
    VMMean = geometric.mean(RuntimeFactor),
    min = min(RuntimeFactor),
    max = max(RuntimeFactor))
...
<<perf-overview-truffle, fig.width=5, fig.height=2, cache=TRUE>>=
p <- overview_box_plot(stats_ns_all, vms_truffle, pre_plot = function (p) { 
...
p + scale_y_continuous(limit=c(0.6,5), breaks = c(1, 1.1, 2, 3, 4, 5)) +
  theme(plot.margin = unit(c(0.05,0.05,0.05,0.05), "cm"))
@


<<perf-overview-fast-vms, fig.width=5, fig.height=3, cache=TRUE>>=
...
<<performance-overview, fig.width=5, fig.height=4, cache=TRUE>>=
