...
```{r all-data, results='asis', class.output="all-data-plots", fig.height=2.5, fig.width=3.5, cache=TRUE}
...
```{r fn-plot-warmup-params, echo=FALSE, cache=FALSE}
...
plot_median_diff_from_hot <- function(b, s, e, legend_height = 20) {
  ...
  for_viz_filtered <- for_viz %>%
    ...
    mutate(diff = diff_to_hot_median_ratio * 100,
           marked = abs((warmup - 1) - (2 * (iter - 1))) <= 25)
...
```{r int-loop-median-diff-from-hot, results='asis', echo=FALSE, fig.height=5, fig.width=7, cache=FALSE}
cp("<figure>")
...
```{r vacation-median-diff-from-hot, results='asis', echo=FALSE, fig.height=5, fig.width=7, cache=FALSE}
cp("<figure>")
...
```{r median-diff-from-hot-all-data, results='asis', class.output="all-data-plots", fig.height=2.5, fig.width=3.5, cache=FALSE}
#options(warn=0)
#warning=FALSE, error=FALSE, 
# b <- "CD", s <- "macro-steady", e <- "SOMns-graal"

# data_ttt <- data_f %>%
#   dplyr::filter(bench == "Havlak", suite == "macro-steady", exe == "SOMns-graal") %>%
#   droplevels()
...
```{r calc-stats, eval = FALSE, echo = FALSE}
# plan(multisession, workers = availableCores() / 2)

diagonal <- params %>%
  filter(abs((warmup - 1) - (2 * (iter - 1))) <= 25) %>%
  mutate(max_iter = warmup + iter)
  # %>%
  # filter(max_iter > 1750, max_iter <= 2000)
...
calc_all_stats <- function(value) {
  ...
  # bca100  <- get_bca(value,    100)
  # bca1000 <- get_bca(value,   1000)
  # bca10000 <- get_bca(value, 10000)
  data.frame(median = median_,
             mean = mean,
             min = min,
             q5 = q[[1]], q10 = q[[2]], q15 = q[[3]], q20 = q[[4]], q25 = q[[5]]
             #,
             #b100.m   = bca100$median,   b100.l   = bca100$lower,   b100.u   = bca100$upper
             #, 
             # b1000.m  = bca1000$median,  b1000.l  = bca1000$lower,  b1000.u  = bca1000$upper,
             # b10000.m = bca10000$median, b10000.l = bca10000$lower, b10000.u = bca10000$upper
             )
}
