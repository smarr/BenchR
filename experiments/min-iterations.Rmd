...
plot_median_diff_from_hot <- function(b, s, e, legend_height = 20) {
  cp("started plot_median_diff_from_hot")
  ...
  for_viz_filtered <- for_viz %>%
    ...
    mutate(diff = diff_to_hot_median_ratio * 100,
           marked = ifelse(abs((warmup - 1) - (2 * (iter - 1))) <= 25, "marked", "unmarked"))
  cp("after filtered")
  summary(for_viz_filtered)
  ...
  plot <- ggplot(for_viz_filtered, aes(x=iter, y=warmup)) +
      ...
      xlab("num. iterations") +
      geom_point(aes(shape = marked),
                 data = for_viz_filtered %>% filter(marked == "marked")) +
      scale_shape_manual(values = c("marked"=3, "unmarked"=FALSE)) +
      theme_simple(8) +
        theme(legend.title = element_text(size = 8))
  cp("after ggplot combine")
  print(plot)
}
```
...
TODO(2020-07-03)
 - mark the steps we want to use for the next experiments


TODO(2020-07-03)
 - add bootstrapped median
 - add bootstrapped upper/lower
   - the dplyr mutate() should allow using multiple return values, I blieve
